...
export default function SampleEventWheel() {
  ...
  {events.map((event, index) => (
    <div className = 'h-40 w-72 border-solid border-black border-2 p-4 rounded-md flex flex-col gap-4 max-w-lg' key={index}>
      <p className = 'underline text-sm'>{event.title.split(" ").length < 10 ? event.title
      : event.title.split(" ").slice(0, 10).join(" ") + ". . ."}</p>
      <p className = 'text-xs'>{event.description.split(" ").length < 30 ? event.description 
      : event.description.split(" ").slice(0, 30).join(" ") + ". . ."}</p>
    </div>
  ))}
  </div>);
}